...
def make_columns_equal(component):
    class NewClass(component):
        ...
        def prox_op(self, v, weight, rho, use_set=None):
            ...
            x_scalar = super().prox_op(
                v_bar, weight, rho, use_set=use_reduced, prox_weights=prox_weights
            )
...
def make_columns_independent(component):
    class NewClass(component):
        ...
        def prox_op(self, v, weight, rho, use_set=None):
            ...
            if use_set is not None:
                x_cols = [
                    f(v[:, j], weight, rho, use_set=use_set[:, j])
                    for j in range(v.shape[1])
                ]
            else:
                x_cols = [
                    f(v[:, j], weight, rho, use_set=None) for j in range(v.shape[1])
                ]
            x = np.r_[x_cols].T
            return x
