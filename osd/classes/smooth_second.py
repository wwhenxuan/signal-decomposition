...
from osd.classes.quadlin_utilities import build_constraint_matrix, build_constraint_rhs
...
class SmoothSecondDifference(QuadLin):
    ...
    def prox_op(self, v, weight, rho, use_set=None, prox_weights=None):
        ...
        if self.P is None:
            ...
            self.F = build_constraint_matrix(n, self.period, self.vavg, self.first_val)
            if self.F is not None:
                self.g = build_constraint_rhs(
                    len(v), self.period, self.vavg, self.first_val
                )
        vout = super().prox_op(
            v, weight, rho, use_set=use_set, prox_weights=prox_weights
        )
        return vout
...
class SmoothSecondDiffPeriodic(SmoothSecondDifference):
    ...
    def prox_op(self, v, weight, rho, use_set=None, prox_weights=None):
        ...
        if self.P is None:
            ...
            self.F = build_constraint_matrix(q, None, self.vavg, self.first_val)
            if self.F is not None:
                self.g = build_constraint_rhs(q, None, self.vavg, self.first_val)
        out_bar = super().prox_op(
            v_bar, weight, rho, use_set=u_bar, prox_weights=prox_weights
        )
