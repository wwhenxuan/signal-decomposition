...
class TimeSmoothEntryClose(QuadLin):
    ...
    def __init__(self, lambda1=1, lambda2=1, quasi_period=None, lambda_qp=1, **kwargs):
        ...
        def costfunc(x):
            ...
            if self.sqrt_P is None:
                if self.quasi_period is None:
                    self.P, self.sqrt_P = make_tsec_mat(
                        T, p, lambda1=self.lambda1, lambda2=self.lambda2
                    )
                else:
    ...
    def prox_op(self, v, weight, rho, use_set=None, prox_weights=None):
        ...
        if self.is_constrained:
            if self.F is None:
                ...
                A = build_constraint_matrix(
                    T, self.period_T, self.vavg_T, self.first_val_T
                )
                A = A.tocoo()
                ...
                self.F = sp.coo_matrix(
                    (data, (i, j)), shape=(left_block.shape[0], left_block.shape[1] + T)
                )
        ...
        if use_set is not None:
            ...
            use_ravel = np.r_[~np.isnan(v_ravel), np.zeros_like(mu, dtype=bool)]
        v_tilde = np.r_[v_ravel, mu]
        # v_tilde[np.isnan(v_tilde)] = 0
        if prox_weights is not None:
            prox_weights = np.r_[prox_weights.ravel(order="F"), np.zeros_like(mu)]
        out_tilde = super().prox_op(
            v_tilde, weight, rho, use_set=use_ravel, prox_weights=prox_weights
        )
...
class TimeSmoothPeriodicEntryClose(TimeSmoothEntryClose):
    def __init__(self, period, lambda1=1, lambda2=1, circular=True, **kwargs):
        ...
        self._internal_constraints = [lambda x, T, p: x[period:, :] == x[:-period, :]]
        return
    ...
    def prox_op(self, v, weight, rho, use_set=None, prox_weights=None):
        ...
        if self.P is None:
            self.P, self.sqrt_P = make_tsec_mat(
                q, p, lambda1=self.lambda1, lambda2=self.lambda2, circular=self.circular
            )
        num_groups = T // q
        use_temp = None
        if T % q != 0:
            ...
            if use_set is not None:
                use_temp = np.r_[use_set, np.zeros((num_new_rows, p), dtype=bool)]
        else:
            v_temp = np.copy(v)
            if use_set is not None:
                use_temp = use_set
        ...
        if np.all(scales == 1):
            scales = None
        out_bar = super().prox_op(
            v_bar, weight, rho, use_set=use_bar, prox_weights=scales
        )
...
def make_quasiper_tsec_mat(
    T, p, period, lambda1=1, lambda2=1, lambda_qp=1, circular=False
):
    _, upper_block = make_tsec_mat(
        T, p, lambda1=lambda1, lambda2=lambda2, circular=circular
    )
    lower_block = lambda_qp * sp.block_diag([make_periodic_A(T, period)] * p)
    zeros = sp.csr_matrix(
        ([], ([], [])),
        shape=(lower_block.shape[0], upper_block.shape[1] - lower_block.shape[1]),
    )
