...
from osd.classes import MeanSquareSmall, AsymmetricNoise, ConstantChunks, LinearTrend
...
class TestColsEqual(unittest.TestCase):
    def test_cols_equal_linear(self):
        ...
        y1 = X1 + Xlt
        y1[~use_set] = np.nan
        c1 = [MeanSquareSmall(size=T * p), make_columns_equal(LinearTrend)]
        p1 = Problem(y1, c1)
    ...
    def test_cols_equal_constant_chunks(self):
        ...
        y3 = X1 + Xch
        y3[~use_set] = np.nan
        c3 = [MeanSquareSmall(size=T * p), make_columns_equal(ConstantChunks)(length=7)]
        p3 = Problem(y3, c3)
