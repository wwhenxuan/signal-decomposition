...
class TestSineSquare(unittest.TestCase):
    def test_cvx(self):
        ...
        np.testing.assert_(np.isclose(problem1.problem.value, opt_obj_val))
    ...
    def test_admm(self):
        ...
        np.testing.assert_(
            ...
            "actual value: {:.3e}".format(rms(problem1.components[2] - X_real[2])),
        )
...
class TestSineSquareMasked(unittest.TestCase):
    def test_cvx(self):
        ...
        rms1 = rms(
            problem1.components[0, problem1.use_set] - X_real[0, problem1.use_set]
        )
        rms2 = rms(
            problem1.components[1, problem1.use_set] - X_real[1, problem1.use_set]
        )
        rms3 = rms(
            problem1.components[2, problem1.use_set] - X_real[2, problem1.use_set]
        )
...
def make_data():
    ...
    signal1 = np.sin(2 * np.pi * t * 1 / (500.0))
    signal2 = signal.square(2 * np.pi * t * 1 / (450.0))
