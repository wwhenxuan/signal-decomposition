...
class Problem:
    ...
    def make_graph_form(self):
        ...
        last_block_row.append(sp.dok_matrix((self.mask.q, Pz.shape[0])))
        ...
        for ix, component in enumerate(self.components):
            for d in component._gx:
                if isinstance(d, dict):
                    new_d = d.copy()
                    new_d["range"] = (self.T * self.p * ix, self.T * self.p * (ix + 1))
                    g.append(new_d)
        z_lengths = [entry.z_size for entry in self.components]
        # print(z_lengths)
    ...
    def decompose(
        self, solver="qss", canonical_form=None, make_feasible=True, **kwargs
    ):
    ...
    def holdout_decompose(
        ...
        **kwargs,
        ...
        residual = self.data[hold_set] - np.sum(self.decomposition, axis=0)[hold_set]
    ...
    def make_feasible_qss(self):
        ...
        use_set = self.mask.use_set
        new_x1[use_set] = (self.data - np.sum(self.decomposition[1:], axis=0))[use_set]
    ...
    def _solve_cvx(self, data, solver, **solver_kwargs):
        ...
        for gfunc in data["g"]:
            if gfunc["g"] == "abs":
                cost += cvx.sum(
                    gfunc["args"]["weight"]
                    * cvx.abs(x[gfunc["range"][0] : gfunc["range"][1]])
                )
            elif gfunc["g"] == "huber":
                try:
                    M = gfunc["args"]["M"]
                ...
                cost += cvx.sum(
                    gfunc["args"]["weight"]
                    * cvx.huber(x[gfunc["range"][0] : gfunc["range"][1]], M=M)
                )
                ...
                constraints.append(
                    x[gfunc["range"][0] : gfunc["range"][1]] >= gfunc["args"]["shift"]
                )
            elif gfunc["g"] == "is_neg":
                constraints.append(
                    x[gfunc["range"][0] : gfunc["range"][1]] <= gfunc["args"]["shift"]
                ...
                constraints.extend(
                    [
                        x[gfunc["range"][0] : gfunc["range"][1]] >= lb,
        ...
        **kwargs,
        ...
        for k in range(K + 1):
            ...
            if k == 0:
                ...
                ax[ax_ix].plot(
                    xs,
                    f(np.sum(self.decomposition[1:], axis=0)),
                    label="denoised estimate",
                    linewidth=1,
                )
                if X_real is not None:
                    ax[ax_ix].plot(
                        xs, f(np.sum(X_real[1:], axis=0)), label="true", linewidth=1
                    )
